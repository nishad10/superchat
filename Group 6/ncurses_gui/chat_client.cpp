...
goto LOOP;
 ...
 i++;
	...
	attach_win = newwin(10, 40, height - 18,width - 40);
        box(attach_win,0,0);
        refresh();
        wrefresh(attach_win);
        move(height - 17,width - 39);
	...
	while(!inFile.eof() && strlen(file_message) < 500) {
		inFile >> file_message;
	}
	chat_message msg;
	msg.body_length(std::strlen(file_message));
        file_message[msg.body_length()] = '1';
        file_message[msg.body_length() + 1] = '\0';
        msg.body_length(std::strlen(file_message));
        std::memcpy(msg.body(), file_message, msg.body_length());
        msg.encode_header();
